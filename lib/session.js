...
function register(params) {
  ...
  if (fs.existsSync(filePath)) {
    try { existing = JSON.parse(fs.readFileSync(filePath, 'utf-8')); } catch { existing = null; }
  }
...
function resolve(explicitId) {
  ...
  return {
    ...
    role: (record && record.role) || envFirst('AIDS_ROLE', 'AID_ROLE', 'ROLE', 'SELFTOOLS_ROLE', 'ZHUYI_ROLE', 'AHA_ROLE') || 'unknown',
    goal: (record && record.goal) || envFirst('AIDS_INTENT', 'AID_INTENT', 'INTENT', 'SELFTOOLS_INTENT', 'ZHUYI_INTENT', 'AHA_TASK_TITLE') || '',
    task_id: (record && record.task_id) || envFirst('AIDS_TASK_ID', 'AID_TASK_ID', 'TASK_ID', 'SELFTOOLS_TASK_ID', 'ZHUYI_TASK_ID', 'AHA_TASK_ID') || '',
    runtime: (record && record.runtime) || envFirst('AIDS_RUNTIME', 'AID_RUNTIME', 'SELFTOOLS_RUNTIME', 'ZHUYI_RUNTIME') || 'unknown',
    actor_type: (record && record.actor_type) || envFirst('AIDS_ACTOR_TYPE', 'AID_ACTOR_TYPE', 'SELFTOOLS_ACTOR_TYPE', 'ZHUYI_ACTOR_TYPE') || 'unknown',
  };
}
