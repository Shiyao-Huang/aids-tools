...
function sleepSync(ms) {
  ...
  while (Date.now() < deadline) {
    // Tiny bounded spin used only while another process owns a session lock.
  }
}

function safeLockName(sessionId) {
  return String(sessionId).replace(/[^A-Za-z0-9_.-]/g, '_');
}

function withSessionLock(sessionId, fn) {
  ensureDir();
  const lockPath = path.join(SESSIONS_DIR, `${safeLockName(sessionId)}.lockdir`);
  ...
  try {
    ...
    try {
      ...
      // Best-effort cleanup; callers will fail after LOCK_TIMEOUT_MS if a lock is stale.
    }
  }
}
...
function writeSessionFileAtomic(filePath, record) {
...
function register(params) {
  ...
  return withSessionLock(params.session_id, () => {
    ...
    if (existing) {
      ...
      writeSessionFileAtomic(filePath, merged);
      return merged;
    }
    ...
    writeSessionFileAtomic(filePath, record);
    return record;
  });
}
...
function heartbeat(sessionId) {
  return withSessionLock(sessionId, () => {
    const filePath = sessionPath(sessionId);
    const record = readSessionFile(filePath);
